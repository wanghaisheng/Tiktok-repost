...
class Crawlers(object):
    def __init__(self):
        ...
        if sys.argv[1] == "test":
            client = MongoClient(
                host=path_config["Mongo_host_local"],
                port=int(path_config["Mongo_port"]),
            )
            ...
            client = MongoClient(
                host=path_config["Mongo_host_local"],
                port=int(path_config["Mongo_port_ten"]),
            )
            self.db = client["handling_vedio"]
            self.collection = self.db["vedios"]
        else:
            client = MongoClient(
                host=path_config["Mongo_host_server"],
                port=int(path_config["Mongo_port"]),
            )
            self.db = client["handling_vedio"]
            self.collection = self.db["vedios"]
        ...
        if crawlers_config["Proxy_switch"] == "False":
            ...
            self.proxy = {
                "http": crawlers_config["Socks5_proxy"],
                "https": crawlers_config["Socks5_proxy"],
            }
        elif crawlers_config["Use_simple_proxy"] == "True":
            self.proxy = {
                "http": "http://" + crawlers_config["Socks5_proxy"],
    ...
    def tiktok_crawler(self, search_keywords):
        ...
        for video in video_list:
            ...
            if self.collection.find_one({"video_id": video_id}) is None or (
                time.time()
                - self.collection.find_one({"video_id": video_id})["video_update_time"]
                > 5 * 60 * 60
            ):
        ...
        if new_video_list:
            params = [
                (None, {"video_id": video["video_id"]}) for video in new_video_list
            ]
            pool = threadpool.ThreadPool(MAX_THREAD)
            tasks = threadpool.makeRequests(
                self.tiktok_video_info, params, self.save_result_tiktok
            )
    ...
    def youtube_crawler(self, search_keywords):
        ...
        if new_video_list is not None:
            ...
            pool = threadpool.ThreadPool(MAX_THREAD)
            tasks = threadpool.makeRequests(
                self.youtube_video_info, params, self.save_result
            )
            [pool.putRequest(req) for req in tasks]
            pool.wait()
            for video in new_video_list:
    ...
    def save_result(self, request, result):
        self.youtube_results.append(
            {"request_id": request.requestID, "results": result}
        )
  ...
  --compressed"
            ...
            if target["status_code"] < 300:
                ...
                for video in videos:
                    ...
                    if re.search(r"(.+?)#", video["desc"]):
                        temp["video_title"] = re.search(r"(.+?)#", video["desc"]).group(
                            1
                        )
    ...
    def youtube_search_video(self, search_keywords):
        ...
        ip = random.choice(
            ["172.53.173.232", "172.105.229.161", "182.125.229.161", "192.53.173.232"]
        )
        ...
        data = {
            "context": {
                "client": {
                    ...
                    "clientName": "WEB",
                    "clientVersion": "2.20221103.04.00",
                    "osName": "Windows",
                    "osVersion": "10.0",
                    ...
                    "screenHeightPoints": 1297,
                    "screenPixelDensity": 1,
                    "screenDensityFloat": 1,
                    "utcOffsetMinutes": 480,
                    "memoryTotalKbytes": "8000000",
                    "mainAppWebInfo": {
                        "graftUrl": "/results?search_query=funny+video",
                ...
                "clickTracking": {
                    "clickTrackingParams": "CA0Q7VAiEwi588CKv5P7AhXQ_TgGHUB3BNI="
                },
                "adSignalsInfo": {
                    "params": [
                        {"key": "dt", "value": "1667529402961"},
                        {"key": "flash", "value": "0"},
                        {"key": "frm", "value": "0"},
                        {"key": "u_tz", "value": "480"},
                        {"key": "u_his", "value": "2"},
        ...
        if res is not None:
            ...
            target = json.loads(res)
            try:
                all_video = target["contents"]["twoColumnSearchResultsRenderer"][
                    "primaryContents"
                ...
                for video in target_video:
                    ...
                    temp = {
                        ...
                        "video_watch_num": video_watch_num,
                        "video_h5_url": video_h5_url,
                        "video_datafrom": video_datafrom,
    ...
    def youtube_video_info(self, video_id):
        ...
        if response is not None:
            json_str = re.findall("var ytInitialPlayerResponse = (.*?);var", response)[
                0
            ]
            ...
            if len(new_list) > 0:
                ...
                for i in range(len(new_list) - 1, -1, -1):
                    if (
                        "audio" in new_list[i]["mimeType"]
                        and "LOW" in new_list[i]["audioQuality"]
                    ):
    ...
    def douyin_search_video(self, search_keyword, offset=0):
        keywords = search_keyword
        search_keyword = quote(search_keyword)
        print(search_keyword)
        baseurl='https://www.douyin.com/aweme/v1/web/general/search/single/?device_platform=webapp&aid=6383&channel=channel_pc_web&search_channel=aweme_general&sort_type=0&publish_time=0&keyword={}&search_source=normal_search&query_correct_type=1&is_filter_search=0&from_group_id=&offset=0&count=10&pc_client_type=1&version_code=190600&version_name=19.6.0&cookie_enabled=true&screen_width=1920&screen_height=1080&browser_language=en-CA&browser_platform=MacIntel&browser_name=Firefox&browser_version=113.0&browser_online=true&engine_name=Gecko&engine_version=109.0&os_name=Mac OS&os_version=10.15&cpu_core_num=4&device_memory=&platform=PC&webid=7224890897015817728&msToken=CpEZa8JCxyYRCSr1KsRDsAyRJt-Irw-Ba6eC0k4F2nO97EcpNWqpVbVz4yh-wmRz01yyP-L_Y3rGGrrtKXYixZoOuwS_ADr6AYL4uj1bpMiF3f1HPwJ8Hz2er0ZvIg==&X-Bogus=DFSzKIVuJt2ANSOctSFCS-jIVUIp'
        ...
        if res is not None:
            ...
            if target["status_code"] < 300:
                ...
                videos = target["data"]
                print('====',len(videos))
                if "aweme_info" in videos==False:
                    print('dou yin api seems to be a little problem, try again later')
                    raise Exception(f"tiktok: 更新: {search_keyword} 失败!!! 接口返回异常")
            ...
            raise Exception(f"tiktok: 更新: {search_keyword} 失败!!! 请检查接口")
    ...
    def simple_post(self, url, data_json, headers):
        ...
        for i in range(5):
            try:
                if self.proxy:
                    res = requests.post(
                        url=url, headers=headers, data=data_json, proxies=self.proxy
                    )
                else:
        ...
        for i in range(5):
            try:
                ...
                if res.status_code < 300:
                    ...
                    print(
                        f"请求 {url} 响应异常 状态码为: {res.status_code} {res.text}, 开始重试, 重试次数:{i + 1}"
                    )
                    time.sleep(random.uniform(0.5, 1))
                    continue
            except Exception as e:
    ...
    def download_video(self):
        ...
        for one in self.collection.find({"video_update_time": {"$gt": diff}}):
            ...
            for i in range(3):
                try:
                    resp = requests.get(
                        url=one["video_url"], headers=headers, stream=True
                    )
